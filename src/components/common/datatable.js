    ...
    datatable() {
        ...
        if (!toolbar) {
            element.parent().find(".dt-toolbar")
                .append('<div class="text-right"><img src="assets/img/logo.jpg" alt="SmartAdmin" style="width: 111px; margin-top: 3px; margin-right: 10px;"></div>');
        }

        if (this.props.buttons) {
            const buttons = this.props.buttons;
            Object.keys(this.props.buttons).map(name => {
                element.on('click', 'td .' + name, function () {
                    const tr = $(this).closest('tr');
                    const row = _dataTable.row(tr);
                    buttons[name](row.data());
                })
            })
        }
